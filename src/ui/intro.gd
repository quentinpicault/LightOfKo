...
func _ready() -> void:
	AudioManager.play()
	AudioManager.switch_track(Utilities.Track.INTRO)
	...
	var tween_show_text3 = create_tween().set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	tween_show_text3.tween_property(text3, "modulate:a", 1.0, 1.0)\
	.set_trans(Tween.TRANS_SINE)\
	.set_ease(Tween.EASE_IN_OUT)

	await get_tree().create_timer(6.0).timeout

	var tween_hide_text3 = create_tween().set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	tween_hide_text3.tween_property(text3, "modulate:a", 0.0, 1.0)\
	.set_trans(Tween.TRANS_SINE)\
	.set_ease(Tween.EASE_IN_OUT)
